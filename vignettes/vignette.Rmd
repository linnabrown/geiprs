...
In addition, it also requires the .pvar.zst file which can be generated using zstd. For example, `zstd yourdata.pvar` will generate `yourdata.pvar.zst`. 

The potential training/validation split can be specified with a separate column in the phenotype file.
...
- `env`: the name of the environment variable. Must be the same as the corresponding column name in the phenotype file. This is required to run the group lasso or sparse group lasso. 
- `tau`: the sparse group lasso mixing parameters. If tau = 0, it is group lasso penalty; if tau =1, it is lasso penalty. Only environment != NULL can consider this parameter. 
...
- `family`: the type of the phenotype: "gaussian". Currently only "gaussian" family is supported.
